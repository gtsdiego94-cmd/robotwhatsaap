...
function gerarEscala() {
  ...
  for (let dia = 1; dia <= 31; dia++) {
    ...
    if (escalaManual[dia]) {
      ...
      escala[dia] = escalaManual[dia];

      escala[dia].folgas = folgasPorDia[dia];
    ...
    const trabalhando = funcionarios.filter(nome => !folgas.includes(nome));

    const giro = (dia - 1) % trabalhando.length;

    const ordem = trabalhando.slice(giro).concat(trabalhando.slice(0, giro));

    postos.forEach((posto, index) => {

      escala[dia][posto] = ordem[index] || "SEM COBERTURA";
